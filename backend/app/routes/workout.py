...
def post_workout_log(log: WorkoutCreate, db: Session = Depends(get_db)):
    db_log = Workout(**log.model_dump(exclude={"sets"}), user_id=1)
    db.add(db_log)
    db.commit()
    db.refresh(db_log)
...
def delete_workout_log(workout_id: int, db: Session = Depends(get_db)):
    db_response = db.query(Workout).filter(Workout.id == workout_id).first()
