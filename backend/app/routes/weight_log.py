...
def post_weight_log(log: WeightLogCreate, db: Session = Depends(get_db)):
    db_log = WeightLog(**log.model_dump(), user_id=1)
    db.add(db_log)
    db.commit()
    db.refresh(db_log)
    return db_log
...
def get_weight_logs(db: Session = Depends(get_db)):
    return db.query(WeightLog).filter(WeightLog.user_id == 1).all()

@router.delete("/{log_id}", )
def delete_weight_log(log_id: int, db: Session = Depends(get_db)):
    db_response = db.query(WeightLog).filter(WeightLog.id == log_id).first()
